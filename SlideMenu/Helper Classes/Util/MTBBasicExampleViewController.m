...
- (void)startScanning {
    ...
    [self.scanner startScanningWithResultBlock:^(NSArray *codes) {
        for (AVMetadataMachineReadableCodeObject *code in codes) {
            if (code.stringValue && [self.uniqueCodes indexOfObject:code.stringValue] == NSNotFound) {
                ...
                NSLog(@"Codigo encontrado: %@", code.stringValue);
                [[NSNotificationCenter defaultCenter]
                 postNotificationName:ESCANEO_EXITOSO
                 object:code.stringValue userInfo:nil];
    ...
    [self.toggleScanningButton setTitle:@"Detener escaneo" forState:UIControlStateNormal];
